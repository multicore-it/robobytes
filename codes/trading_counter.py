...
#파라미터 설정
### for real
## open_amt_list = [0.0, 9.0, 21.6, 39.24, 63.936, 98.5104, 146.9146, 214.6804] #단계별 주문수량
# revenue_rate = 0.013  #익절 비율
# max_loss_rate = 0.8      #손절 비율: 80%
# increace_rate = 0.4   #수량 증가 비율
...
### for test
# revenue_rate = 0.0005  #익절 비율
# max_loss_rate = 0.0005   #손절 비율: 20%
# increace_rate = 0.1   #수량 증가 비율
# open_cnt_limit = 2    #최대 오픈 건수
# open_amt_unit = 9     #최소 오픈 수량

revenue_rate = 0.013  #익절 비율
max_loss_rate = 0.15   #손절 비율: 15%
increace_rate = 0.45
open_cnt_limit = 5   #최대 오픈 건수
open_amt_unit = 9     #최소 오픈 수량
open_amt_list = []    #단계별 주문수량
open_amt_list = util.get_open_amt_list(open_amt_unit, open_cnt_limit, increace_rate)
max_loss = 0          #손절 가격(stop loss 주문시 계산)
...
while True:
    ...
    ######(1) 분데이터 가져오기, 분데이터는 6초후 생성 start ######
    ...
    ######(2) position open start ######
    if trade_flag == 0 and check_ss in ('58','59','00','01'):

        trade_flag = 1
        ###(2)-1 cancel before timestep order ###
        if long_order_id != "":
        ...
        # print("***try open short amt:{} open_short_cnt:{} close:{} vwap:{}".
        #       format(amt, open_short_cnt, close, vwap))
    ...
    ######(3) take profit start #####
    ###(3)-1 prepare take profit
    # take profit 주문이 체결되었다면 profit amt를 0으로 만들어서 새로운 take profit 주문을 준비한다.
    print("*profit_long_id:{} profit_short_id:{}".format(profit_long_id, profit_short_id))
    if profit_long_id != "":
        status, message = trade.ask_order_status(request_client, coin_name, profit_long_id)
        print("*profit_long status:{} status:{}".format(status, message))
        if message != 'good':
    ...
    if profit_short_id != "":
        ...
        # print("*profit_short status:{} status:{}".format(status, message))
        if message != 'good':
    ...
    ###(3)-2 LONG Take Profit start ###
    amt, entry_price, message = trade.get_position_amt(request_client, coin_name, 'long')
    if message != 'good':
        util.log_info("+[{}]long take profit-get_position_amt error msg:{}".
                      format(util.get_time_hhmmss(time.time()), message))
        continue
    # print("*amt:{} profit_long_amt:{}".format(amt,profit_long_amt))
    ...
    ###(3)-3 SHORT Take Profit start ###
    amt, entry_price, message = trade.get_position_amt(request_client, coin_name, 'short')
    diff_amt = amt - profit_short_amt
    # print("*amt:{} profit_short_amt:{}".format(amt, profit_short_amt))
    if diff_amt > 0:
        # 이전 주문 취소, 모든 주문을 취소하고 전체를 다시 주문한다.
        if profit_short_id != "":
